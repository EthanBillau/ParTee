 ...
 *
 * Unit test suite for Reservations data model and state management.
 * Tests reservation creation, property access, and state transitions.
 *
 * Data structures: Reservations objects with various configurations, payment/pending flags.
 * Algorithm: JUnit 5 with assertion-based verification of state changes.
 * Features: Reservation instantiation, property getters/setters, payment status tracking,
 * approval state management, object equality and hashing.
 *
...
public class ReservationsTest {
    ...
    void setUp() {
        ...
        testReservation = new Reservations("R001", "maya_user", "2024-11-15", 
                                          "10:00 AM", 4, "Hole 1", 120.00);
    }
    ...
    void testConstructor() {
        Reservations res = new Reservations("R002", "john_doe", "2024-11-16",
                                           "2:00 PM", 2, "Hole 10", 80.00);
        assertNotNull(res, "Reservation object should not be null");
    }
    
    @Test
    void testConstructorMinPartySize() {
        Reservations res = new Reservations("R003", "solo_golfer", "2024-11-20",
                                           "7:00 AM", 1, "Hole 1", 40.00);
        assertEquals(1, res.getPartySize(), "Party size should be 1");
    }
    
    @Test
    void testConstructorMaxPartySize() {
        Reservations res = new Reservations("R004", "big_group", "2024-11-20",
                                           "9:00 AM", 4, "Hole 1", 160.00);
        assertEquals(4, res.getPartySize(), "Party size should be 4");
    }
    
    @Test
    void testConstructorZeroPrice() {
        Reservations res = new Reservations("R005", "free_play", "2024-11-20",
                                           "6:00 AM", 2, "Hole 1", 0.00);
    ...
    void testToStringDifferentReservation() {
        Reservations res2 = new Reservations("R999", "jane_smith", "2024-12-01",
                                             "8:00 AM", 1, "Hole 5", 60.00);
        String result = res2.toString();
        assertTrue(result.contains("R999") && result.contains("jane_smith"),
