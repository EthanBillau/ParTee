 ...
 *
 * Unit test suite for MakeReservationGUI constructor and component initialization.
 * Tests date/time selection, hole selection, party size, cart and event options.
 *
 * Data structures: MakeReservationGUI instance, various Swing components for
 * date/time, hole grid, checkboxes, combo boxes, and navigation buttons.
 * Algorithm: JUnit 5 with reflection-based component verification for both
 * new reservation mode and edit reservation mode.
 * Features: Constructor validation (new and edit modes), component initialization,
 * frame properties, date field, time button, hole buttons array, party size combo,
 * cart checkbox, event checkbox, continue/back buttons per project requirements.
 *
...
public class MakeReservationGUITest {
    ...
    public void testFramePropertiesNewReservation() {
        assertEquals("Make Reservation", makeReservationGUI.getTitle(),
                "Frame title should be 'Make Reservation' for new bookings");
        assertEquals(JFrame.DISPOSE_ON_CLOSE, makeReservationGUI.getDefaultCloseOperation(),
    ...
    public void testConstructorEditMode() {
        ...
        assertEquals("Edit Reservation", editGUI.getTitle(),
    ...
    public void testCalendarInviteCheckBoxExists() throws Exception {
        Field calendarInviteCheckBox = MakeReservationGUI.class.getDeclaredField(
    ...
    public void testEditingReservationField() throws Exception {
        Field editingReservationField = MakeReservationGUI.class.getDeclaredField(
                "editingReservation"
        );
        editingReservationField.setAccessible(true);
        Reservations editingReservation = (Reservations) editingReservationField.get(
                makeReservationGUI
        );
    ...
    public void testFrameIsVisibleAfterConstruction() {
        assertTrue(makeReservationGUI.isVisible(),
