 ...
 *
 * <p>JUnit test suite for EventApprovalGUI class. Tests constructors and basic GUI component
 * initialization for event approval. Does NOT test event handlers (button clicks) per project
 * requirements.
 *
...
public class EventApprovalGUITest {
  ...
  /** Test that EventApprovalGUI is modal. Modal dialogs block interaction with parent window. */
  @Test
  ...
  /**
   * Test that EventApprovalGUI has a content pane with components. Should have list/table of
   * pending events and approve/reject buttons.
   */
  @Test
  ...
  public void testEventApprovalGUITitleContainsEventKeywords() {
    ...
    boolean hasEventKeyword =
        title.contains("event")
            || title.contains("approval")
            || title.contains("pending")
            || title.contains("request");

    assertTrue(hasEventKeyword, "EventApprovalGUI title should contain event-related keywords");
    gui.dispose();
    parent.dispose();
  }
  ...
  public void testEventApprovalGUIDispose() {
    JFrame parent = new JFrame();
    EventApprovalGUI gui = new EventApprovalGUI(parent);
    assertDoesNotThrow(() -> gui.dispose(), "EventApprovalGUI should dispose without exceptions");
    parent.dispose();
  }

  /**
   * Test that EventApprovalGUI handles database initialization. This tests that the GUI initializes
   * correctly even with empty event list.
   */
  @Test
