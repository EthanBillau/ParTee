 ...
 * ServerTest.java
 *
 * <p>Unit test suite for Server startup, client connection, and shutdown operations. Tests
 * multi-client handling and graceful server termination.
 *
 * <p>Data structures: Server instances, client connection threads, ServerSocket management.
 * Algorithm: JUnit 5 with thread testing, socket connection validation, lifecycle verification.
 * Features: Server startup on port, client connection acceptance, worker thread spawning, server
 * shutdown, resource cleanup, thread interrupt handling.
 *
 * @author Nikhil Kodali (kodali3), Anoushka Chakravarty (chakr181), Connor Landzettel (clandzet),
 *     L15
...
public class ServerTest {
  ...
  //       and inspect private fields 'running' (should be false) and `serverSocket` (should be
  // null).
  @Test
  void testConstructorInitializesFields() {
    ...
    assertNull(
        getPrivate(serverUnderTest, "serverSocket"), "Server socket should be null before start");
  }

  // Test: start() causes server to listen and set running=true.
  // How: Start server, wait until 'running' becomes true (polling). Then verify serverSocket is
  // non-null
  ...
  void testStopStopsServerAndInterruptsWorkerThreads() throws Exception {
    ...
    serverUnderTest.start();
    assertTrue(
        waitForRunning(serverUnderTest, 2000), "Server should be running before creating clients");
    ...
    assertTrue(
        workers.size() >= 2,
        "At least two worker threads should have been created after two connections");
    ...
    assertFalse(
        getPrivateBoolean(serverUnderTest, "running"),
        "Server running flag should be false after stop()");

    ServerSocket ss = (ServerSocket) getPrivate(serverUnderTest, "serverSocket");
    // serverSocket may be closed but still referenced; check closed condition if non-null
    if (ss != null) {
      assertTrue(
          ss.isClosed() || !ss.isBound(), "Server socket should be closed or unbound after stop()");
    }
  ...
  // Test: calling start() when already running does not create a second server main thread nor
  // crash.
  // How: Start server, wait for running true, then call start() again and assert no exception and
  // still running.
  @Test
  void testDoubleStartDoesNotCrashWhenAlreadyRunning() throws Exception {
    ...
    serverUnderTest.start();
    assertTrue(
        waitForRunning(serverUnderTest, 2000), "Server should be running after first start()");
    ...
    assertTrue(
        getPrivateBoolean(serverUnderTest, "running"),
        "Server should remain running after second start()");
  }
  ...
  void testStopBeforeStartIsSafe() {
    ...
    assertDoesNotThrow(
        () -> serverUnderTest.stop(), "Calling stop() before start() should not throw");
    assertFalse(
        getPrivateBoolean(serverUnderTest, "running"),
  ...
  void testConcurrentStopsAreSafe() throws Exception {
    ...
    serverUnderTest.start();
    assertTrue(
        waitForRunning(serverUnderTest, 2000), "Server should be running before concurrent stop()");
  ...
  // How: Start server, open three client sockets sequentially, then inspect workerThreads list and
  // assert size >= 3.
  @Test
  void testMultipleClientsSpawnWorkerThreads() throws Exception {
    ...
    serverUnderTest.start();
    assertTrue(
        waitForRunning(serverUnderTest, 2000),
        "Server should be running before connecting clients");
