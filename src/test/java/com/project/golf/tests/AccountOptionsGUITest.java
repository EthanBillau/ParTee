 ...
 *
 * <p>Comprehensive JUnit test suite for AccountOptionsGUI class. Tests constructors, inheritance,
 * interface implementation, window properties, layout, components, visibility, and lifecycle for
 * account editing interface. Does NOT test event handlers (button clicks) per project requirements.
 *
...
public class AccountOptionsGUITest {
  ...
  public void testAccountOptionsGUIHasReasonableSize() {
    ...
    assertTrue(size.width > 0 && size.height > 0, "AccountOptionsGUI should have a non-zero size");
    assertTrue(
  ...
  public void testAccountOptionsGUIUsesBoxLayout() {
    ...
    assertTrue(
        layout instanceof BoxLayout || contentPane.getComponentCount() > 0,
  ...
  public void testAccountOptionsGUIHasMultipleButtons() {
    ...
    assertTrue(
        buttonCount >= 6,
        "AccountOptionsGUI should have at least 6 buttons (change username, password, first name,"
            + " last name, email, back)");
    gui.dispose();
  }
  ...
  public void testAccountOptionsGUIHasMultipleTextFields() {
    ...
    assertTrue(textFieldCount >= 5, "AccountOptionsGUI should have at least 5 text fields");
    gui.dispose();
  }
  ...
  public void testAccountOptionsGUIHasMultipleLabels() {
    ...
    assertTrue(
        labelCount >= 5, "AccountOptionsGUI should have multiple labels for field descriptions");
    gui.dispose();
  }
  ...
  public void testAccountOptionsGUITitleRelevant() {
    ...
    assertNotNull(title);
    assertTrue(
        title.toLowerCase().contains("golf")
            || title.toLowerCase().contains("account")
            || title.toLowerCase().contains("options")
  ...
  public void testAccountOptionsGUIDispose() {
    AccountOptionsGUI gui = new AccountOptionsGUI("usertest", null);
    assertDoesNotThrow(
        () -> gui.dispose(), "AccountOptionsGUI should be disposable without throwing exception");
  }
