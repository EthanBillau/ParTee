 ...
 * NoAccountGUI.java
 *
 * <p>Displays when user lacks account authorization and handles account requests. Guides users to
 * request account creation through email submission.
 *
 * <p>Data structures: JButton for navigation and email submission, JTextField for email address
 * input. Algorithm: Email validation and submission to admin system for account creation, UI
 * switching between login and account request screens. Features: Account request submission, email
 * validation, admin notification, return to login navigation, email-based access control
 * integration.
 *
...
public class NoAccountGUI extends JFrame implements ActionListener {
  ...
  /** Handle account creation Validates form inputs and creates new user account */
  private void handleAccountCreation() {
    ...
    if (username.isEmpty()
      ...
      JOptionPane.showMessageDialog(
          this, "All fields are required.", "Validation Error", JOptionPane.WARNING_MESSAGE);
      return;
    }
    ...
    if (!password.equals(confirmPassword)) {
      JOptionPane.showMessageDialog(
          this, "Passwords do not match.", "Validation Error", JOptionPane.WARNING_MESSAGE);
      return;
    }
    ...
    new Thread(
            () -> {
              try {
                boolean success =
                    client.addUser(username, password, firstName, lastName, email, false);
