 ...
 *
 * <p>User account profile editing interface for customers. Allows modification of username,
 * password, name, and email with validation.
 *
 * <p>Data structures: User object for account data, UserManager for persistence, Client for server
 * communication. JButton array for edit options, JTextField/JPasswordField for input capture.
 * AccountBackgroundPanel for themed rendering. Algorithm: Field-by-field update with password
 * verification, dual-field password confirmation, server synchronization of changes. Features:
 * Username/password/email/name editing, password verification, form validation, profile
 * persistence, background image scaling, rollback on cancel.
 *
...
public class AccountOptionsGUI extends JFrame implements ActionListener {
        ...
        if (response != null && response.startsWith("RESP|OK|")) {
          ...
          if (currentUser == null) {
            JOptionPane.showMessageDialog(
                this, "Failed to load user data from server.", "Error", JOptionPane.ERROR_MESSAGE);
          }
        } else {
          JOptionPane.showMessageDialog(
              this,
  ...
  private void handleChangePassword() {
    ...
    if (!currentPwd.equals(currentUser.getPassword())) {
      JOptionPane.showMessageDialog(
          this, "Current password does not match.", "Error", JOptionPane.ERROR_MESSAGE);
      return;
    }
  ...
  public static void main(String[] args) {
    ...
    SwingUtilities.invokeLater(() -> new AccountOptionsGUI("defaultUser", client));
  }
  ...
  private void showAccountOptionsScreen() {
    ...
    confirmPasswordField.setMinimumSize(new Dimension(25, usernameField.getPreferredSize().height));
    ...
    firstNameField.setMaximumSize(new Dimension(250, usernameField.getPreferredSize().height * 2));
    firstNameField.setMinimumSize(new Dimension(25, usernameField.getPreferredSize().height));
    if (currentUser != null) {
    ...
    lastNameField.setMaximumSize(new Dimension(250, usernameField.getPreferredSize().height * 2));
    lastNameField.setMinimumSize(new Dimension(25, usernameField.getPreferredSize().height));
    if (currentUser != null) {
    ...
    emailField.setMaximumSize(new Dimension(250, usernameField.getPreferredSize().height * 2));
    emailField.setMinimumSize(new Dimension(25, usernameField.getPreferredSize().height));
    if (currentUser != null) {
