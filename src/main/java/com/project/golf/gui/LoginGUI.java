 ...
 * LoginGUI.java
 *
 * <p>Primary authentication interface for ParTee golf reservation system. Handles user login with
 * username/password, password reset via one-time codes, account creation for new users, and
 * navigation to main application.
 *
 * <p>Data structures: JTextField for credentials, JPasswordField for secure password input, Session
 * variables for one-time password recovery mechanism. Algorithm: Validates credentials against
 * server, manages user sessions, routes to appropriate GUI screens. Features: Login, account
 * creation, password recovery, show/hide password toggle.
 *
 * @author Ethan Billau (ebillau), Connor Landzettel (clandzet), Anoushka Chakravarty (chakr181),
 *     L15
...
public class LoginGUI extends JFrame implements ActionListener {
  ...
  public LoginGUI() {
    ...
    usernameField.setMaximumSize(
        new Dimension(300, usernameField.getPreferredSize().height * 2)); // Increased from 250
  ...
  public void actionPerformed(ActionEvent e) {
    if (e.getSource() == signupButton) {
      ...
      /**
       * Toggle password visibility. If currently hidden, reveal the password by removing the echo
       * char. If currently shown, hide it again with bullet points.
       */
  ...
  private void handleForgotPassword() {
    /**
     * Prompt user for their username to send password reset code. Validates that user exists before
     * sending code.
     */
    String inputUsername =
        JOptionPane.showInputDialog(
            this, "Enter your username:", "Forgot Password", JOptionPane.QUESTION_MESSAGE);
    ...
    /** Get user's email from database. If user doesn't exist or has no email, show error. */
    new Thread(
            () -> {
              String email = null;
              ...
              if (email == null) {
                com.project.golf.database.Database db =
                    com.project.golf.database.Database.getInstance();
              ...
              /**
               * Generate 6-digit one-time code. Store it for validation when user tries to log in.
               */
              oneTimeCode = String.format("%06d", (int) (Math.random() * 1000000));
              oneTimeCodeUsername = username;

              /** Send email with one-time code. Display message based on success/failure. */
              boolean emailSent =
                  com.project.golf.utils.EmailSender.sendEmail(
                      userEmail,
  ...
  private void attemptLogin(String u, String p) {
    /**
     * Check if user is trying to log in with one-time code. If code matches, allow login without
     * checking password.
     */
    if (oneTimeCode != null
        && oneTimeCodeUsername != null
        && oneTimeCodeUsername.equals(u)
        && oneTimeCode.equals(p)) {
    ...
    /**
     * Run login in background thread to avoid freezing the UI. Connect to server and validate
     * credentials.
     */
    new Thread(
            () -> {
              try {
                ...
                /**
                 * Switch back to UI thread for any GUI updates. This prevents threading issues with
                 * Swing.
                 */
                SwingUtilities.invokeLater(
                    () -> {
                      if (response != null && response.startsWith("RESP|OK")) {
                         ...
                         */
                        String message =
                            "Your account has not been paid.\n"
                                + "Please contact the golf course to complete payment.";
                        if (response.split("\\|").length > 3) {
                          message = response.split("\\|", 4)[3];
                        }
                        JOptionPane.showMessageDialog(
                            this, message, "Payment Required", JOptionPane.WARNING_MESSAGE);
                      } else {
                        /**
                         * Show error dialog with two options. User can either try logging in again
                         * or go to the no account screen.
                         */
                        int choice =
    ...
    submitButton.addActionListener(
        e -> {
          ...
          if (newPassword.isEmpty()) {
            JOptionPane.showMessageDialog(
                passwordDialog, "Password cannot be empty.", "Error", JOptionPane.ERROR_MESSAGE);
            return;
          }

          if (!newPassword.equals(confirmPassword)) {
            JOptionPane.showMessageDialog(
                passwordDialog, "Passwords do not match.", "Error", JOptionPane.ERROR_MESSAGE);
            return;
          }
                  ...
                  () -> {
                    try {
                      ...
                      if (response != null && response.startsWith("RESP|OK|")) {
                        ...
                        com.project.golf.users.User user =
                            com.project.golf.users.User.fromFileString(userData);
