 ...
 *
 * <p>Displays and manages user's existing golf course reservations and events. Provides table view
 * with edit/delete actions and new reservation creation.
 *
 * <p>Data structures: JTable with DefaultTableModel for reservation display, JPanel layouts for
 * content organization, Lists for reservation/event data, String username and Client for server
 * synchronization. Algorithm: Table-driven display pattern with action button event handling,
 * dynamic refresh with server queries, column customization for readability. Features: Reservation
 * viewing and management, edit/delete actions, event display, creation navigation, refresh
 * capability, server data synchronization.
 *
...
public class ManageReservationsGUI extends JFrame implements ActionListener {
  ...
  private JPanel buildTablePanel() {
    ...
    tableModel =
        new DefaultTableModel(null, colNames) {
          @Override
  ...
  private void loadReservations() {
    ...
    try {
      ...
      /**
       * Show appropriate view based on whether user has any reservations or pending events. Empty
       * state shows a message, otherwise display the table.
       */
  ...
  public void showWithReservationsAndPending(
      List<Reservations> reservations, List<com.project.golf.events.Event> pendingEvents) {
  ...
  private void editReservation(String reservationId) {
    ...
    if (reservation == null) {
      JOptionPane.showMessageDialog(
          this, "Reservation not found.", "Error", JOptionPane.ERROR_MESSAGE);
      return;
    }
  ...
  private boolean deleteReservation(String reservationId) {
    /** Confirm with user before deleting. Deletion is permanent and can't be undone. */
    ...
    try {
      ...
      if (client != null) {
        ...
        if (removed) {
          ...
          JOptionPane.showMessageDialog(
              this, "Failed to delete reservation.", "Error", JOptionPane.ERROR_MESSAGE);
          return false;
        }
      }
    } catch (Exception e) {
  ...
  private class ActionsCellEditor extends AbstractCellEditor implements TableCellEditor {
    ...
    ActionsCellEditor() {
      ...
      deletePendingBtn = new JButton("Delete");
      styleActionButton(deletePendingBtn);
      deletePendingBtn.addActionListener(
          e -> {
            fireEditingStopped();
            deleteReservation(currentReservationId);
          });
