 ...
 *
 * Complex reservation creation interface with date/time selection and event options.
 * Handles regular tee time bookings and multi-hole event reservations with calendar UI.
 *
 * Data structures: JTextField for dateField, JButton array holeButtons, JComboBox
 * for partySizeCombo, JCheckBox for cartCheckBox and eventCheckBox, multiple JPanels.
 * Algorithm: Calendar dialog for date picking, time picker for tee times, grid layout
 * for hole selection, event expansion logic. Dynamic UI updates based on event flag.
 * Features: Date/time selection with UI dialogs, party size configuration, cart rental,
 * event booking with all holes, calendar invite generation, server synchronization.
 *
 * @author Anoushka Chakravarty (chakr181), Ethan Billau (ebillau), Connor Landzettel (clandzet), L15
 *
...
public class MakeReservationGUI extends JFrame implements ActionListener {
    ...
    /**
     * Pre-fills the reservation form with existing reservation data when editing.
     * Sets date, time, hole, and party size from the editingReservation object.
     */
    ...
    private void saveReservation() {
        try {
            ...
            if (!isEvent) {
                ...
                String editingResId = (editingReservation != null) ? editingReservation.getReservationId() : null;
            ...
            if (client != null) {
                try {
                    String editingResId = (editingReservation != null) ?
                                           editingReservation.getReservationId() : null;
                    String response = client.createReservation(username, formattedDate, time,
                                                                partySize, teeBox, 0.0, editingResId);
                    ...
                    if (response.startsWith("RESP|OK|")) {
                         ...
                         * Retrieve user's email from server and send iCalendar invite.
                         */
                        if (sendCalendarInvite) {
                            try {
                                ...
                                if (userEmail != null && !userEmail.isEmpty()) {
                                    ...
                                    new Thread(() -> {
                                        boolean emailSent = EmailSender.sendCalendarInvite(
                                ...
                                System.err.println("Error getting user email for calendar invite: " + e.getMessage());
                            }
                        }
                        ...
                        JOptionPane.showMessageDialog(this,
                            String.format("Reservation %s!\n\nReservation ID: %s\nDate: %s\nTime: %s\nHole: %d\nParty Size: %d%s%s",
                                action, serverResId, date, time, selectedHole, partySize, cartInfo, calendarInfo),
                            "Success",
                            JOptionPane.INFORMATION_MESSAGE);
                ...
                // Local mode - direct database access
                Reservations reservation = new Reservations(reservationId, username, formattedDate,
                                                             time, partySize, teeBox, 0.0, false);
