 ...
 * UserManager.java
 *
 * <p>Manages user account data with thread-safe file persistence. Provides operations for user
 * creation, authentication, lookup, and deletion. Automatically loads existing users from disk on
 * initialization.
 *
 * <p>Data structures: ArrayList of User objects, ReentrantReadWriteLock for synchronization.
 * Algorithm: Linear search for user lookup, file I/O for persistence. Features: User CRUD
 * operations, login validation, thread-safe read/write access.
 *
...
public class UserManager implements UserManagerInterface {
  ...
  /** Constructor for UserManager Initializes the users list */
  public UserManager() {
    users = new ArrayList<>();
    try {
  ...
  /**
   * Adds a new user to the manager if username doesn't exist Thread safe with write lock. Persists
   * change to file. Now hashes passwords before storing and checks for duplicate emails.
   *
  ...
  /**
   * Searches for a user by username Thread safe with read lock
   *
  ...
  /**
   * Searches for a user by email Thread safe with read lock
   *
  ...
  /**
   * Validates user login credentials Thread safe with read lock Supports login with username or
   * email Uses BCrypt to verify hashed passwords, falls back to plaintext for legacy data
   *
  ...
  /**
   * Removes a user by username Thread safe with write lock. Persists change to file.
   *
  ...
  /**
   * Gets a copy of all users Thread safe with read lock. Returns a copy to prevent external
   * modification.
   *
  ...
  /**
   * Persists all users to disk file Thread safe with read lock. Writes all user data as
  ...
  /**
   * Loads users from disk file Thread safe with write lock. Parses each line into User objects. If
   * file doesn't exist, starts with empty user list.
   *
